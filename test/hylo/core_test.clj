...
(deftest function
  ...
  (is (= (mk-fn Double [Double])
         (:type (hylo-fn (fn [x] (hylo.core/sqrt x))))))
  (is (= (mk-fn :a [:a :a])
         (:type (hylo-fn (fn [x y] (if true x y))))))
  (is (= (mk-fn Double [Double Double Boolean])
         (:type (hylo-fn (fn [x y z] (if z x (hylo.core/sqrt y)))))))
  (is (= (mk-fn Double [Double Double Boolean])
         (:type (hylo-fn (fn [x y z] (if z (hylo.core/sqrt y) x))))))
  (is (= (mk-fn Double [(mk-fn Double [:a]) :a])
         (:type (hylo-fn (fn [f x] (hylo.core/sqrt (f x))))))))
